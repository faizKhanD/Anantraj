...
import investorTypeModel from "../../models/investorType.model";
...
import { InvestorTypesSchema ,UpdateInvestorTypesSchema} from "../../validation/admin/investorType.validate";

export const Store = asyncHandler(async (req, res, next) => {
  try {
    const { error } = InvestorTypesSchema.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      return next({
    ...
    const resepnse = await investorTypeModel.create({
      name: req.body.name,
      status: req.body.status ?? 1,
    });

    successResponse(res, "Type created successfully", resepnse);
  } catch (error: any) {
...
export const Update = asyncHandler(
  async (
    ...
    try {
      const { error } = UpdateInvestorTypesSchema.validate(req.body, {
        abortEarly: false,
      });
      if (error) {
        return next({
      ...
      const { id } = req.params;
      const response = await investorTypeModel.findByPk(id);
      if (!response) {
        return next({ status: 404, message: "Type not found" });
      }

      await response.update(req.body);

      successResponse(res, "Type updated successfully", response);
    } catch (error: any) {
      logger.error(`Database Error: ${error.message}`);
      next(error);
    }
